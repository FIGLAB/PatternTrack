...
for fname in images:
    ...
    if ret == True:
        ...
        image = put_pattern_on_image(gray,corners)
...
import json
np.savez(f"./{args.path}/intrinsic_calib.npz", mtx=K, dist=D)
with open("./{}/per_instrinsic.json".format(args.path), "w") as f:
    json.dump(data, f)
# [Calibration]
...
## [Load]
import json
json_data=open(f"./{args.path}/per_instrinsic.json").read()
data = json.loads(json_data)
scaled_K, D, new_K, dim3 = np.asarray(data['scaled_K']), np.asarray(data['D']), np.asarray(data['new_K']), tuple(data['dim3'])

scaled_K_ = np.copy(scaled_K)
scaled_K_[0, 0] = scaled_K_[0, 0]  # scale
scaled_K_[1, 1] = scaled_K_[1, 1]  # scale
## [Load]


## [Apply Perspective Projection]
filepath = "./{}/00025.jpg".format(args.path)
Knew = scaled_K.copy()
new_K[(0,1), (0,1)] = 2 * Knew[(0,1), (0,1)]
map1, map2 = cv2.fisheye.initUndistortRectifyMap(scaled_K, D, np.eye(3), scaled_K_, dim3, cv2.CV_16SC2)
